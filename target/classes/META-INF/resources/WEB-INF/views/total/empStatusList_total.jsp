...
<head>
    ...
    <title> 투입 인력 현황: IYF 영업관리시스템</title>
...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;"> 투입된 인력 전체 현황 [투입인력현황] </span>
				</div>
...
<!--         </section> -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            // 그리드 데이터 ajax로 가져오기
           $.ajax({
                url: "${pageContext.request.contextPath}/empStatus.ajax",
                method: "GET",
                success: function (empStatusList) {
                    // Date 년월일 Formatting
                    empStatusList.forEach(empStatus => {
                        if (empStatus.birthdate) {
                            const birthdate = new Date(empStatus.birthdate);
                            empStatus.birthdate = birthdate.getFullYear() + '-' + String(Number(birthdate.getMonth() + 1)).padStart(2, '0') + '-' + String(birthdate.getDate()).padStart(2, '0');
                        }
                        if (empStatus.assign_date) {
                            const assign_date = new Date(empStatus.assign_date);
                            empStatus.assign_date = assign_date.getFullYear() + '-' + String(Number(assign_date.getMonth() + 1)).padStart(2, '0') + '-' + String(assign_date.getDate()).padStart(2, '0');
                        }
                        if (empStatus.end_date) {
                            const end_date = new Date(empStatus.end_date);
                            empStatus.end_date = end_date.getFullYear() + '-' + String(Number(end_date.getMonth() + 1)).padStart(2, '0') + '-' + String(end_date.getDate()).padStart(2, '0');
                        }
                        if (empStatus.resume_submit_date) {
                            const resume_submit_date = new Date(empStatus.resume_submit_date);
                            empStatus.resume_submit_date = resume_submit_date.getFullYear() + '-' + String(Number(resume_submit_date.getMonth() + 1)).padStart(2, '0') + '-' + String(resume_submit_date.getDate()).padStart(2, '0');
                        }
                        if (empStatus.resign_date) {
                            const resign_date = new Date(empStatus.resign_date);
                            empStatus.resign_date = resign_date.getFullYear() + '-' + String(Number(resign_date.getMonth() + 1)).padStart(2, '0') + '-' + String(resign_date.getDate()).padStart(2, '0');
                        }
                        if (empStatus.i_contract_date) {
                            const i_contract_date = new Date(empStatus.i_contract_date);
                            empStatus.i_contract_date = i_contract_date.getFullYear() + '-' + String(Number(i_contract_date.getMonth() + 1)).padStart(2, '0') + '-' + String(i_contract_date.getDate()).padStart(2, '0');
                        }
                        if (empStatus.c_contract_date) {
                            const c_contract_date = new Date(empStatus.c_contract_date);
                            empStatus.c_contract_date = c_contract_date.getFullYear() + '-' + String(Number(c_contract_date.getMonth() + 1)).padStart(2, '0') + '-' + String(c_contract_date.getDate()).padStart(2, '0');
                        }
                        
                     	// 휴대폰번호 Formatting
                        if (empStatus.phonenumber) {
                        	empStatus.phonenumber = empStatus.phonenumber.substr(0,3) + '-' + empStatus.phonenumber.substr(3, 4) + '-' + empStatus.phonenumber.substr(7,4);
                        }	
                    });
                    grid.resetData(empStatusList);
                }
            });
