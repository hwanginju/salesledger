...
public class IYCNCCONVERSIONPageController {
	...
	/***
	 * @기능 iycnc/conversion/salesCostStatusList.jsp 매출원가현황 목록페이지
	 */
	@PreAuthorize("hasAuthority('통합 관리자') or hasAuthority('IYCNC 관리자')")
	@GetMapping("/iycnc/conversion/salesCostStatusList")
	public String salesCostStatusListPage(ModelMap modelMap) {
		if (log.isInfoEnabled()) {log.info("Start IYCNCCONVERSIONPageController.salesCostStatusListpPage");}
		modelMap.addAttribute("company", "IYCNC");
		modelMap.addAttribute("company_lower", "iycnc");
		modelMap.addAttribute("department", "CONVERSION");
		modelMap.addAttribute("department_lower", "conversion");
		...
		if (log.isInfoEnabled()) {log.info("modelMap ::: " + modelMap);}
		if (log.isInfoEnabled()) {log.info("page ::: " + "iycnc/conversion/salesCostStatusList");}
		if (log.isInfoEnabled()) {log.info("End IYCNCCONVERSIONPageController.salesCostStatusListpPage");}
		return "iycnc/conversion/salesCostStatusList";
	}

    /***
	 * @기능 iycnc/idc/salesCostStatusDetail.jsp 매출원가현황 목록 상세페이지
	 */
	@PreAuthorize("hasAuthority('통합 관리자') or hasAuthority('IYCNC 관리자')")
	@GetMapping("/iycnc/conversion/salesCostStatusDetail")
	public String salesCostStatusDetailPage(@RequestParam long emp_id, ModelMap modelMap) {
		if (log.isInfoEnabled()) {log.info("Start IYCNCCONVERSIONPageController.salesCostStatusDetailPage");}
		modelMap.addAttribute("company", "IYCNC");
		modelMap.addAttribute("company_lower", "iycnc");
		modelMap.addAttribute("department", "CONVERSION");
		modelMap.addAttribute("department_lower", "conversion");
		...
		if (log.isInfoEnabled()) {log.info("modelMap ::: " + modelMap);}
		if (log.isInfoEnabled()) {log.info("page ::: " + "iycnc/conversion/salesCostStatusDetail");}
		if (log.isInfoEnabled()) {log.info("End IYCNCCONVERSIONPageController.salesCostStatusDetailPage");}
		return "iycnc/conversion/salesCostStatusDetail";
	}
}
