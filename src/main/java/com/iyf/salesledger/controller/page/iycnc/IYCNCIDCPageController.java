...
public class IYCNCIDCPageController {
	...
	/***
	 * @기능 iycnc/idc/salesCostStatusList.jsp 매출원가현황 목록페이지
	 */
	@PreAuthorize("hasAuthority('통합 관리자') or hasAuthority('IYCNC 관리자')")
	@GetMapping("/iycnc/idc/salesCostStatusList")
	public String salesCostStatusListPage(ModelMap modelMap) {
		...
		modelMap.addAttribute("department_kr", "IDC");
		if (log.isInfoEnabled()) {log.info("modelMap ::: " + modelMap);}
		if (log.isInfoEnabled()) {log.info("page ::: " + "iycnc/idc/salesCostStatusList");}
		if (log.isInfoEnabled()) {log.info("End IYCNCIDCPageController.salesCostStatusListpPage");}
		return "iycnc/idc/salesCostStatusList";
	}
	 ...
	 */
	@PreAuthorize("hasAuthority('통합 관리자') or hasAuthority('IYCNC 관리자')")
	@GetMapping("/iycnc/idc/salesCostStatusDetail")
	public String salesCostStatusDetailPage(@RequestParam long emp_id, ModelMap modelMap) {
		if (log.isInfoEnabled()) {log.info("Start IYCNCIDCPageController.salesCostStatusDetailPage");}
		modelMap.addAttribute("company", "IYCNC");
		modelMap.addAttribute("company_lower", "iycnc");
		modelMap.addAttribute("department", "IDC");
		modelMap.addAttribute("department_lower", "idc");
		...
		if (log.isInfoEnabled()) {log.info("modelMap ::: " + modelMap);}
		if (log.isInfoEnabled()) {log.info("page ::: " + "iycnc/idc/salesCostStatusDetail");}
		if (log.isInfoEnabled()) {log.info("End IYCNCIDCPageController.salesCostStatusDetailPage");}
		return "iycnc/idc/salesCostStatusDetail";
	}
}
