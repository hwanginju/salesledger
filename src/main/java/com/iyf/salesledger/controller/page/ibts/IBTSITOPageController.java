...
public class IBTSITOPageController {
	...
	/***
	 * @기능 ibts/ito/salesCostStatusList.jsp 매출원가현황 목록페이지
	 */
	@PreAuthorize("hasAuthority('통합 관리자') or hasAuthority('IBTS 관리자')")
	@GetMapping("/ibts/ito/salesCostStatusList")
	public String salesCostStatusListPage(ModelMap modelMap) {
		if (log.isInfoEnabled()) {log.info("Start ITBSITOPageController.salesCostStatusListpPage");}
		modelMap.addAttribute("company", "IBTS");
		modelMap.addAttribute("company_lower", "ibts");
		modelMap.addAttribute("department", "ITO");
		modelMap.addAttribute("department_lower", "ito");
		modelMap.addAttribute("department_kr", "ITO");
		if (log.isInfoEnabled()) {log.info("modelMap ::: " + modelMap);}
		if (log.isInfoEnabled()) {log.info("page ::: " + "ibts/ito/salesCostStatusList");}
		if (log.isInfoEnabled()) {log.info("End IBTSITOPageController.salesCostStatusListpPage");}
		return "ibts/ito/salesCostStatusList";
	}

    /***
	 * @기능 ibts/ito/salesCostStatusDetail.jsp 매출원가현황 목록 상세페이지
	 */
	@PreAuthorize("hasAuthority('통합 관리자') or hasAuthority('IBTS 관리자')")
	@GetMapping("/ibts/ito/salesCostStatusDetail")
	public String salesCostStatusDetailPage(@RequestParam long emp_id, ModelMap modelMap) {
		if (log.isInfoEnabled()) {log.info("Start IBTSITOPageController.salesCostStatusDetailPage");}
		modelMap.addAttribute("company", "IBTS");
		modelMap.addAttribute("company_lower", "ibts");
		modelMap.addAttribute("department", "ITO");
		modelMap.addAttribute("department_lower", "ito");
		...
		if (log.isInfoEnabled()) {log.info("modelMap ::: " + modelMap);}
		if (log.isInfoEnabled()) {log.info("page ::: " + "ibts/ito/salesCostStatusDetail");}
		if (log.isInfoEnabled()) {log.info("End IBTSITOPageController.salesCostStatusDetailPage");}
		return "ibts/ito/salesCostStatusDetail";
	}
}
