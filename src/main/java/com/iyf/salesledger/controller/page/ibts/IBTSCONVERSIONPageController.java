...
public class IBTSCONVERSIONPageController {
	...
	/***
	 * @기능 ibts/conversion/salesCostStatusList.jsp 매출원가현황 목록페이지
	 */
	@PreAuthorize("hasAuthority('통합 관리자') or hasAuthority('IBTS 관리자')")
	@GetMapping("/ibts/conversion/salesCostStatusList")
	public String salesCostStatusListPage(ModelMap modelMap) {
		if (log.isInfoEnabled()) {log.info("Start ITBSCONVERSIONPageController.salesCostStatusListpPage");}
		modelMap.addAttribute("company", "IBTS");
		modelMap.addAttribute("company_lower", "ibts");
		modelMap.addAttribute("department", "CONVERSION");
		modelMap.addAttribute("department_lower", "conversion");
		...
		if (log.isInfoEnabled()) {log.info("modelMap ::: " + modelMap);}
		if (log.isInfoEnabled()) {log.info("page ::: " + "ibts/conversion/salesCostStatusList");}
		if (log.isInfoEnabled()) {log.info("End IBTSCONVERSIONPageController.salesCostStatusListpPage");}
		return "ibts/conversion/salesCostStatusList";
	}

    /***
	 * @기능 ibts/conversion/salesCostStatusDetail.jsp 매출원가현황 목록 상세페이지
	 */
	@PreAuthorize("hasAuthority('통합 관리자') or hasAuthority('IBTS 관리자')")
	@GetMapping("/ibts/conversion/salesCostStatusDetail")
	public String salesCostStatusDetailPage(@RequestParam long emp_id, ModelMap modelMap) {
		if (log.isInfoEnabled()) {log.info("Start IBTSCONVERSIONPageController.salesCostStatusDetailPage");}
		modelMap.addAttribute("company", "IBTS");
		modelMap.addAttribute("company_lower", "ibts");
		modelMap.addAttribute("department", "CONVERSION");
		modelMap.addAttribute("department_lower", "conversion");
		...
		if (log.isInfoEnabled()) {log.info("modelMap ::: " + modelMap);}
		if (log.isInfoEnabled()) {log.info("page ::: " + "ibts/conversion/salesCostStatusDetail");}
		if (log.isInfoEnabled()) {log.info("End IBTSCONVERSIONPageController.salesCostStatusDetailPage");}
		return "ibts/conversion/salesCostStatusDetail";
	}
}
