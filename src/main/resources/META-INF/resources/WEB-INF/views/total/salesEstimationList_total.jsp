...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;">${company} 전체 <span id="year"></span>년 <span id="month"></span>월 용역 매출 현황 [매출추정]</span>
				</div>
                ...
                <div class="toggle-content mt-3" id="toggle-item">
                    ...
                    <div class="checkbox-container mb-3">
                        ...
                        <div class="row row-cols-4 mb-3">
                            ...
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="사업부서" checked>
                                    <span class="form-check-label">사업부서</span>
                                </label>
                            </div>
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="사이트" checked>
                                    <span class="form-check-label">사이트명</span>
                                </label>
                            </div>
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="진행업체" checked>
                                    <span class="form-check-label">진행업체</span>
                                </label>
                            </div>
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="프로젝트명" checked>
                                    <span class="form-check-label">프로젝트명</span>
                                </label>
                            </div>
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="적요란" checked>
                                    <span class="form-check-label">적요란</span>
                                </label>
                            </div>
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="매출가" checked>
                                    <span class="form-check-label">매출가</span>
                                </label>
                            </div>
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="매입가" checked>
                                    <span class="form-check-label">매입가</span>
                                </label>
                            </div>
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="이익" checked>
                                    <span class="form-check-label">이익</span>
                                </label>
                            </div>
                            <div class="col">
                                <label class="form-check">
                                    <input class="form-check-input" type="checkbox" data-table="sales-result" name="column" value="수기작성여부" checked>
...
<!--         </section> -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/salesEstimationTotal.ajax/company/${company}/batch_month/" + yyyymm,
                method: "GET",
                success: function (salesEstimationTotal) {
                	salesEstimationTotal.forEach(item => {
                		item.이익 = Number(item.매출가) - Number(item.매입가);
                	});
                    grid.resetData(salesEstimationTotal);
            ...
            function updateColumns(dataTable) {
         	...
         	// 그리드 Row 더블 클릭시 이벤트: 그리드 상단 총합계 데이터 alert
            grid.on('dblclick', function (ev) {
               const estimation = grid.getRow(ev.rowKey)
               
               if (!estimation.sales_estimation_id) {
               	alert('총합계 데이터입니다')
            ...
            $("#toggle-icon").click(function () {
       		...
       		$('#btnKeywordSearch').click(function() {
                ...
                if (regex.test($('#keyword').val())) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath}/salesEstimationTotal.ajax/company/${company}/batch_month/" + $('#keyword').val(),
                        method: "GET",
                        success: function (salesEstimationTotal) {
                        	salesEstimationTotal.forEach(item => {
                                item.이익 = Number(item.매출가) - Number(item.매입가);
       		...
       		$('#btnManualBatch').click(function() {
       			...
       			console.log("company ::: " + company + " ::: " + "batch_month ::: " + batch_month);
       			
       			if (!confirm(company + ' ' + ' 매출추정의 ' + $('#year').text() + '년 ' + $('#month').text() + '월' + ' 수동배치를 실행하시겠습니까?')) {
       				return;	
       			}
       			
       			$.ajax({
                    type: "POST",
                    url: "${pageContext.request.contextPath}/salesEstimationTotal.ajax/company/" + company +  "/batch_month/" + batch_month,
                    success: function() {
