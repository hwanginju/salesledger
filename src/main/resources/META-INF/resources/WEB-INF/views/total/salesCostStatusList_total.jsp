...
<!--             <div class="container"> -->
				<div id="title" class="d-flex justify-content-center" style="width: 80vw;">
			  		<span style="font-size: 20px; font-weight: 550; color: #012970c7; font-family: 'Nunito', sans-serif;">${company} 전체 <span id="year"></span>년 <span id="month"></span>월 매출 원가 현황</span>
				</div>
...
<!--         </section> -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            // 그리드 데이터 ajax로 가져오기
            $.ajax({
                url: "${pageContext.request.contextPath}/salesCostStatusTotal.ajax/company/${company}/batch_month/" + yyyymm,
                method: "GET",
                success: function (selesCostStatusListTotal) {
                	selesCostStatusListTotal.forEach(item => {
                    ...
                    grid.resetData(selesCostStatusListTotal);
            ...
            $("#toggle-icon").click(function () {
       		...
       		$('#btnKeywordSearch').click(function() {
                ...
                if (regex.test($('#keyword').val())) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath}/salesCostStatusTotal.ajax/company/${company}/batch_month/" + $('#keyword').val(),
                        method: "GET",
      		...
      		// 수동배치 클릭이벤트
       		$('#btnManualBatch').click(function() {
       			...
       			if (!confirm(company + ' '  + ' 매출 원가 현황의 ' + $('#year').text() + '년 ' + $('#month').text() + '월' + ' MM 자동계산을 실행하시겠습니까?')) {
       				return;	
       			}
